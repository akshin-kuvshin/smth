...
public:
    ...
    lli first_above(lli val, lli l, lli x, lli lx, lli rx) {
        if (rx <= l)
            return BAD;
        if (tree[x] < val)
            return BAD;
        ...
        lli mid = (lx + rx) / 2LL,
            res;
        if ((res = first_above(val, l, 2LL * x + 1LL, lx, mid)) != BAD)
            return res;
        res = first_above(val, l, 2LL * x + 2LL, mid, rx);
        return res;
    }

    lli first_above(lli val, lli l) {
        return first_above(val, l, 0LL, 0LL, size);
    }
};
